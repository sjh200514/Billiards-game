...
void Cushion::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    painter->setBrush(QColor(Qt::green).lighter(60));
    //绘制中间绿色运动区域
    ...
    painter->setBrush(QColor(Qt::white).lighter(70));
    //绘制中间区域的点与线
    painter->drawPath(m_pointPath);

    painter->setPen(Qt::black);
    painter->setBrush(QColor(Qt::green).lighter(50));
    //绘制库边矩形与圆角
    painter->drawPath(m_cushionPath + m_ellipsePath);
    painter->setBrush(QColor(150, 75, 0).lighter(70));

    QPainterPath outFramePath;
    outFramePath.addRoundedRect(-m_outRect.x()/2.6/2, -m_outRect.y()/2.6/2, m_outRect.x()/2.6, m_outRect.y()/2.6, 3, 5);
    ...
    linear.setColorAt(0, QColor(150, 75, 0).lighter(70));
    linear.setColorAt(0.3, QColor(150, 75, 0).lighter(120));
    linear.setColorAt(1,QColor(150, 75, 0).lighter(70));
//    linear.setSpread(QGradient::ReflectSpread);
    painter->setBrush(linear);
    //绘制外边框（一个大的圆角矩形减去中间一个矩形）
    painter->drawPath(outFramePath - inFramePath);
    painter->setPen(QColor(Qt::white).lighter(70));
